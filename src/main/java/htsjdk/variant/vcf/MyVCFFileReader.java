...
public class MyVCFFileReader implements Closeable, Iterable<VariantContext> {
	...
	public static boolean isBCF(final String url) {
		return (url.endsWith(".bcf") ||
                        url.endsWith(".bcf.gz") ||
                        url.endsWith(".bcf.cip") ||
                        url.endsWith(".bcf.gz.cip"));
	}
...
//	public VCFFileReader(final SeekableStream stream) {
		//this(file, true);
//	}
...
//    public VCFFileReader(final SeekableStream stream, final SeekableStream indexStream) {
        //this(file, indexFile, true);
//    }
